"""
True Multi-modal Fusion Model
Fuses chest X-ray image features with clinical notes text features
at the representation level — before the classification head.

Architecture:
    X-ray  → CNN backbone (ResNet/DenseNet) → image_features  (2048-d)
    Notes  → SentenceTransformer            → text_features   (384-d)
               ↓                                    ↓
           Projection(2048→512)           Projection(384→512)
                            ↓              ↓
                        Concat (1024-d)
                            ↓
                    Fusion MLP → 14 logits

Compared to the post-hoc keyword boost in doctor_notes.py, this model
learns which visual patterns co-occur with which clinical descriptions
directly from data.

Training:
    Use FusionTrainer to fine-tune the projection + fusion head while
    keeping the backbone frozen (or lightly unfrozen).

Inference:
    Pass (image_tensor, notes_text) → sigmoid → 14 disease probabilities.
    Works gracefully when notes_text is empty (zero text vector).
"""
...
class TextEncoder(nn.Module):
    """
    Wraps a SentenceTransformer to encode free-text notes into a
    fixed-length embedding vector.

    The encoder is kept frozen by default — its weights are not updated
