...
Runs N stochastic forward passes with dropout active to estimate:
  ...
  - Std deviation per disease (epistemic uncertainty)
  - Flags predictions that are uncertain near the decision boundary
"""
...
def mc_dropout_predict(
    ...
    """
    Run N stochastic forward passes with dropout kept active.

    Args:
        model:        trained model with Dropout layers
        image_tensor: preprocessed X-ray (1, 3, H, W) or (3, H, W)
        n_passes:     number of MC samples (30 is a good default)
        device:       inference device; auto-detected if None

    Returns:
        mean_probs: {disease: mean probability across passes}
        std_probs:  {disease: std deviation}  ← higher = more uncertain
    """
    if device is None:
