"""
Model Ensemble — Weighted averaging of ResNet, DenseNet, and ViT predictions.

Ensembling the three trained models consistently improves AUC compared to
any single model, with no additional training required.

Usage:
    from models.ensemble import EnsembleModel, load_ensemble

    ensemble = load_ensemble(
        resnet_ckpt   = "models/federated/resnet_final.pth",
        densenet_ckpt = "models/federated/densenet_final.pth",
        vit_ckpt      = "models/federated/vit_final.pth",
    )
    mean_probs, individual_probs = ensemble.predict(image_tensor)
"""
...
class EnsembleModel:
    """
    Soft-voting ensemble over an arbitrary list of trained models.

    Each model gets a weight (default: equal weights).  The final probability
    for each disease is the weighted average of per-model probabilities.

    Args:
        models:  list of trained nn.Module objects (eval mode expected)
        weights: optional list of floats, one per model; auto-normalised
        names:   optional list of model name strings for reporting
